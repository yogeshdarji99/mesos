...
public:
  ...
  virtual void initialize()
  {
    ...
    //
    // (1) `sessionTimeout` is the client's proposed value for the Zk
    // session timeout.
    //
    // (2) `initLoopTimeout` is how long we are prepared to wait,
    // calling `zookeeper_init` in a loop, until a call succeeds.
    //
    // `sessionTimeout` is used to determine the liveness of our Zk
    // session. `initLoopTimeout` determines how long to retry
    // erroneous calls to `zookeeper_init`, because there are cases
    // when temporary DNS outages cause `zookeeper_init` to return
    // failure. ZooKeeper masks EAI_AGAIN as EINVAL and a name
    ...
    // success but we don't see a subsequent Zk event indicating that
    // our connection has been established. A common cause for this
    // situation is that the Zk hostname list resolves to unreachable
    // IP addresses. Zk will continue looping, trying to connect to
    // the list of IPs but not attempting to re-resolve the input
    // hostnames. Since DNS information may have changed, we close the
    // Zk handle and start a new Zk handle, which ensures Zk will try
    // to re-resolve the configured Zk hostnames. However, since we
    // can't easily check for whether the `connected` Zk event has
    // fired for this session yet, we implement this timeout in
    // `GroupProcess`. See MESOS-4546 for more information.
