...
void Slave::doReliableRegistration()
{
  ...
  if (info.id() == "") {
    ...
    list<Future<ExecutorInfo> > infos;
    ...
    foreachvalue (Framework* framework, frameworks) {
      foreachvalue (Executor* executor, framework->executors) {
        ...
        if (!executor->commandExecutor) {
          infos.push_back(executor->info);
        }
      }
    }

    collect(infos).then(defer(self(),
                              &Self::_doReliableRegistration,
                              lambda::_1));
  }
    ...
    const list<ExecutorInfo>& infos)
{
  ...
  foreach(const ExecutorInfo& info, infos) {
    ExecutorInfo* executorInfo = message.add_executor_infos();
    executorInfo->CopyFrom(info);
    ...
    const ExecutorID& executorId)
{
  ...
  switch (executor->state) {
    ...
    case Executor::REGISTERING: {
      ...
      // Will effectively be the same as future.get() and we can rely
      // on executor->info being ready.
...
Future<ExecutorInfo> Slave::executorLaunched(
    const FrameworkID& frameworkId,
    const ExecutorID& executorId,
    const ContainerID& containerId,
    const TaskInfo& task,
    const Future<ExecutorInfo>& future)
{
  if (!future.isReady()) {
    ...
    return Failure(
        "Container '" + stringify(containerId) + "' for executor '" +
        stringify(executorId) + "' of framework '" + stringify(frameworkId) +
        "' failed to start: " +
        (future.isFailed() ? future.failure() : " future discarded"));
  }
  ...
  if (framework == NULL) {
    return Failure(
        "Framework '" + stringify(frameworkId) + "' for executor '" +
        stringify(executorId) + "' is no longer valid");
  }
  ...
  if (framework->state == Framework::TERMINATING) {
    ...
    return Failure(
        "Killing executor '" + stringify(executorId) + "' of framework '" +
        stringify(frameworkId) + "' because the framework is terminating");
  }
  ...
  if (executor == NULL) {
    containerizer->destroy(containerId);
    return Failure(
        "Killing unknown executor '" + stringify(executorId) + "' of framework'"
  ...
  if (task.has_executor() && !(executorInfo.executor_id() == executorId)) {
    containerizer->destroy(containerId);
    return Failure(
        "Executor id is invalid. Found '" +
        stringify(executorInfo.executor_id()) + "' and expected '" +
  ...
  if (executor->checkpoint) {
    ...
    paths::createExecutorDirectory(
        metaDir, info.id(), frameworkId, executorId, containerId);
  }
  ...
  switch (executor->state) {
    case Executor::TERMINATING:
      ...
      return Failure(
          "Killing executor '" + stringify(executorId) + "' of framework '" +
          stringify(frameworkId) + "' because the executor is terminating");
    break;
    case Executor::REGISTERING:
    case Executor::RUNNING:
    ...
    default:
      return Failure(
          "Executor '" + stringify(executorId) + "' of framework '" +
          stringify(frameworkId) + "' is in an unexpected state " +
          stringify(executor->state));
    break;
  }
...
Future<Nothing> Slave::_recover()
{
  foreachvalue (Framework* framework, frameworks) {
    foreachvalue (Executor* executor, framework->executors) {
      // Only launched executors will be recovered. Executors in
      // flight will be destroyed.
      CHECK(executor->info.isReady());
...
Executor* Framework::launch(const TaskInfo& task) {
  ...
  executor->info = slave->containerizer->launch(
    ...
    .then(defer(slave,
                ...
                task,
                lambda::_1))
    .onFailed(lambda::bind(fail, lambda::_1));
