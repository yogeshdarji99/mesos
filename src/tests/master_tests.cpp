...
TEST_F(MasterTest, StateSummaryEndpoint)
{
  ...
  ASSERT_EQ(
      1u,
      state.values["slaves"].as<JSON::Array>().values.at(0).is<JSON::Object>());

  JSON::Object slaveJson =
    state.values["slaves"].as<JSON::Array>().values.at(0).as<JSON::Object>();
  ASSERT_TRUE(slaveJson.values["TASK_RUNNING"].is<JSON::Number>());
  EXPECT_EQ(0U, slaveJson.values["TASK_RUNNING"].as<JSON::Number>());
  ASSERT_TRUE(slaveJson.values["TASK_KILLED"].is<JSON::Number>());
  EXPECT_EQ(1U, slaveJson.values["TASK_KILLED"].as<JSON::Number>());


  ASSERT_TRUE(state.values["frameworks"].is<JSON::Array>());
  ASSERT_EQ(1u, state.values["frameworks"].as<JSON::Array>().values.size());

  ASSERT_EQ(1u, state.values["frameworks"].as<JSON::Array>()
                .values.at(0).is<JSON::Object>());

  JSON::Object frameworkJson = state.values["frameworks"].as<JSON::Array>()
      .values.at(0).as<JSON::Object>();
  ASSERT_TRUE(frameworkJson.values["TASK_RUNNING"].is<JSON::Number>());
  EXPECT_EQ(0U, frameworkJson.values["TASK_RUNNING"].as<JSON::Number>());
  ASSERT_TRUE(frameworkJson.values["TASK_KILLED"].is<JSON::Number>());
  EXPECT_EQ(1U, frameworkJson.values["TASK_KILLED"].as<JSON::Number>());
