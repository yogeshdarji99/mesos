...
struct TaskStateSummary
{
  ...
  // Computes counts for each 'TaskState' for all the tasks in the
  // given framework. This is not exact due to the circular buffer
  // used to store completed tasks.
  TaskStateSummary(const Framework& framework)
    : staging(0),
      starting(0),
      running(0),
      finished(0),
      ...
      lost(0),
      error(0) {
    // TODO(jmlvanre): use a delegating default constructor.
    staging = framework.pendingTasks.size();
...
// Generates a mapping of 'SlaveID' to counts for each 'TaskState' for
// all slaves. This is not exact due to the circular buffer used to
// store completed tasks.
hashmap<SlaveID, TaskStateSummary> summarizeTaskStates(
    const hashmap<FrameworkID, Framework*>& frameworks)
{
  hashmap<SlaveID, TaskStateSummary> summary;

  foreachvalue (Framework* framework, frameworks) {
    foreachvalue (const TaskInfo& taskInfo, framework->pendingTasks) {
      summary[taskInfo.slave_id()].staging++;
    }

    foreachvalue (const Task* task, framework->tasks) {
      summary[task->slave_id()].count(*task);
    }
...
Future<Response> Master::Http::stateSummary(const Request& request)
{
  ...
  // Model all of the slaves.
  {
    ...
    hashmap<SlaveID, TaskStateSummary> summaries =
      summarizeTaskStates(master->frameworks.registered);

    foreachvalue (Slave* slave, master->slaves.registered) {
      JSON::Object json = model(*slave);

      // Add the task state summary for this slave.
      TaskStateSummary summary;
      if (summaries.contains(slave->id)) {
        summary = summaries[slave->id];
      }
  ...
  // Model all of the frameworks.
  {
    ...
    foreachpair (const FrameworkID& frameworkId,
      ...
      // Add the 'taskstate' summary.
      const TaskStateSummary summary(*framework);
      json.values["TASK_STAGING"] = summary.staging;
      json.values["TASK_STARTING"] = summary.starting;
      json.values["TASK_RUNNING"] = summary.running;
      json.values["TASK_FINISHED"] = summary.finished;
